...
function handleGenTable(row) {
  ...
  if (row.genType === "1") {
    genCode(row.tableName).then(response => {
      proxy.$modal.success("成功生成到自定义路径：" + row.genPath);
    });
  } else {
    proxy.$download.zip("/tool/gen/batchGenCode?tables=" + tbNames, "jar.zip");
  }
}
/** 同步数据库操作 */
